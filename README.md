...
- 刷新最近 N 天首播剧集的媒体库元数据；
- 可选调用 MoviePilot 整理链路尝试重新整理剧集文件；
- 支持选择媒体服务器和维护媒体库；
- 支持媒体服务器到 MoviePilot 的媒体路径映射；
- 支持试运行、单次最大处理数量和执行摘要通知。

配置项：执行周期，最近 N 天，媒体服务器，维护媒体库，路径映射，试运行模式，单次最大处理数量。

详细配置说明和推荐测试步骤见：[最近剧集维护插件说明](plugins.v2/recentepisodemaintenance/README.md)

## 使用建议

首次使用建议开启试运行模式，并将单次最大处理数量设置为 1，确认媒体服务器查询、路径映射和 MoviePilot 整理链路正常后再正式启用。
